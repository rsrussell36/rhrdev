<?php

class rhr_permalink_frontend {
	
	private $rhr_rhr_parse_request_status = false;

	private $rhr_query_string_uri = '';

	private $rhr_registered_url = '';

	private $request_uri = '';

	public function __construct() {
		if ( isset( $_SERVER['QUERY_STRING'] ) ) {
			$this->rhr_query_string_uri = $_SERVER['QUERY_STRING'];
		}

		if ( isset( $_SERVER['REQUEST_URI'] ) ) {
			$this->request_uri = $_SERVER['REQUEST_URI'];
		}

		add_action( 'template_redirect', array( $this, 'rhr_make_redirect' ), 5 );

		add_filter( 'request', array( $this, 'rhr_parse_request' ) );
		add_filter( 'oembed_request_post_id', array( $this, 'rhr_oembed_request' ), 10, 2 );
		add_filter( 'post_link', array( $this, 'rhr_custom_post_link' ), 10, 2 );
		add_filter( 'post_type_link', array( $this, 'rhr_custom_post_link' ), 10, 2 );
		add_filter( 'page_link', array( $this, 'rhr_custom_page_link' ), 10, 2 );
		add_filter( 'term_link', array( $this, 'rhr_custom_term_link' ), 10, 2 );
		add_filter( 'user_trailingslashit', array( $this, 'rhr_custom_trailingslash' ) );

		// WPSEO Filters.
		add_filter(
			'wpseo_canonical',
			array( $this, 'rhr_fix_canonical_double_slash' ),
			20,
			1
		);
	}

	/**
	 * Replace double slash `//` with single slash `/`.
	 */
	public function rhr_remove_double_slash( $permalink = '' ) {
		$protocol = '';
		if ( 0 === strpos( $permalink, 'http://' )
			|| 0 === strpos( $permalink, 'https://' )
		) {
			$split_protocol = explode( '://', $permalink );
			if ( 1 < count( $split_protocol ) ) {
				$protocol  = $split_protocol[0] . '://';
				$permalink = str_replace( $protocol, '', $permalink );
			}
		}
		$permalink = str_replace( '//', '/', $permalink );
		$permalink = $protocol . $permalink;

		return $permalink;
	}

	/**
	 * Use `wpml_permalink` to add language information to permalinks and
	 */
	public function rhr_wpml_permalink_filter( $permalink, $language_code ) {
		$rhr_permalink   = $permalink;
		$trailing_permalink = trailingslashit( home_url() ) . $rhr_permalink;
		if ( $language_code ) {
			$permalink = apply_filters(
				'wpml_permalink',
				$trailing_permalink,
				$language_code
			);
			$site_url  = site_url();
			$wpml_href = str_replace( $site_url, '', $permalink );
			if ( 0 === strpos( $wpml_href, '//' ) ) {
				if ( 0 !== strpos( $wpml_href, '//' . $language_code . '/' ) ) {
					$permalink = $site_url . '/' . $language_code . '/' . $rhr_permalink;
				}
			}
		} else {
			$permalink = apply_filters( 'wpml_permalink', $trailing_permalink );
		}
		return $permalink;
	}

	/**
	 * Search a permalink in the posts table and return its result.
	 */
	private function rhr_query_post( $requested_url ) {
		global $wpdb;

		$cache_name = 'cp$_' . str_replace( '/', '-', $requested_url ) . '_#cp';
		$posts      = wp_cache_get( $cache_name, 'rhr_permalinks' );

		if ( ! $posts ) {
			$posts = $wpdb->get_results(
				$wpdb->prepare(
					'SELECT p.ID, pm.meta_value, p.post_type, p.post_status ' .
					" FROM $wpdb->posts AS p INNER JOIN $wpdb->postmeta AS pm ON (pm.post_id = p.ID) " .
					" WHERE pm.meta_key = 'rhr_permalink' " .
					' AND (pm.meta_value = %s OR pm.meta_value = %s) ' .
					" AND p.post_status != 'trash' AND p.post_type != 'nav_menu_item' " .
					" ORDER BY FIELD(post_status,'publish','private','pending','draft','auto-draft','inherit')," .
					" FIELD(post_type,'post','page') LIMIT 1",
					$requested_url,
					$requested_url . '/'
				)
			);

			$remove_like_query = apply_filters( 'cp_remove_like_query', '__true' );
			if ( ! $posts && '__true' === $remove_like_query ) {
				$posts = $wpdb->get_results(
					$wpdb->prepare(
						"SELECT p.ID, pm.meta_value, p.post_type, p.post_status FROM $wpdb->posts AS p " .
						" LEFT JOIN $wpdb->postmeta AS pm ON (p.ID = pm.post_id) WHERE " .
						" meta_key = 'rhr_permalink' AND meta_value != '' AND " .
						' ( LOWER(meta_value) = LEFT(LOWER(%s), LENGTH(meta_value)) OR ' .
						'   LOWER(meta_value) = LEFT(LOWER(%s), LENGTH(meta_value)) ) ' .
						"  AND post_status != 'trash' AND post_type != 'nav_menu_item'" .
						' ORDER BY LENGTH(meta_value) DESC, ' .
						" FIELD(post_status,'publish','private','pending','draft','auto-draft','inherit')," .
						" FIELD(post_type,'post','page'), p.ID ASC LIMIT 1",
						$requested_url,
						$requested_url . '/'
					)
				);
			}

			wp_cache_set( $cache_name, $posts, 'rhr_permalinks' );
		}

		return $posts;
	}

	/**
	 * Check conditions if it matches then return true to stop processing the
	 * particular query like for sitemaps.
	 */
	private function rhr_exclude_query_proccess( $query ) {
		$exclude = false;

		/*
		 * Return Query for Sitemap pages.
		 */
		if ( isset( $query )
			&& (
				( isset( $query['sitemap'] ) && ! empty( $query['sitemap'] ) )
				|| (
					isset( $query['seopress_sitemap'] )
					&& ! empty( $query['seopress_sitemap'] )
				)
				|| (
					isset( $query['seopress_cpt'] )
					&& ! empty( $query['seopress_cpt'] )
				)
				|| (
					isset( $query['seopress_sitemap_xsl'] )
					&& 1 === (int) $query['seopress_sitemap_xsl']
				)
			)
		) {
			$exclude = true;
		}

		return $exclude;
	}

	/**
	 * Filter to rewrite the query if we have a matching post.
	 */
	public function rhr_parse_request( $query ) {
		global $wpdb;

		if ( isset( $_SERVER['REQUEST_URI'] )
			&& $_SERVER['REQUEST_URI'] !== $this->request_uri
		) {
			// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			$this->request_uri = $_SERVER['REQUEST_URI'];
		}

		/*
		 * Return Query for Sitemap pages.
		 */
		$stop_query = $this->rhr_exclude_query_proccess( $query );
		if ( $stop_query ) {
			// Making it true to avoid redirect if query doesn't needs to be processed.
			$this->rhr_rhr_parse_request_status = true;
			return $query;
		}

		/*
		 * First, search for a matching custom permalink, and if found generate the
		 * corresponding original URL.
		 */
		$original_url = null;

		// Get request URI, strip parameters and /'s.
		$url     = wp_parse_url( get_bloginfo( 'url' ) );
		$url     = isset( $url['path'] ) ? $url['path'] : '';
		$request = ltrim( substr( $this->request_uri, strlen( $url ) ), '/' );
		$pos     = strpos( $request, '?' );
		if ( $pos ) {
			$request = substr( $request, 0, $pos );
		}

		if ( ! $request ) {
			return $query;
		}

		$ignore = apply_filters( 'rhr_permalinks_request_ignore', $request );

		if ( '__true' === $ignore ) {
			return $query;
		}

		if ( defined( 'POLYLANG_VERSION' ) ) {
			$cp_form = new rhr_Permalinks_Form();
			$request = $cp_form->check_conflicts( $request );
		}

		$request_no_slash  = preg_replace( '@/+@', '/', trim( $request, '/' ) );
		$posts             = $this->rhr_query_post( $request_no_slash );
		$permalink_matched = false;

		if ( $posts ) {
			/*
			 * A post matches our request. Preserve this url for later use. If it's
			 * the same as the permalink (no extra stuff).
			 */
			if ( trim( $posts[0]->meta_value, '/' ) === $request_no_slash ) {
				$this->rhr_registered_url = $request;
				$permalink_matched    = true;
			}

			if ( 'draft' === $posts[0]->post_status ) {
				if ( 'page' === $posts[0]->post_type ) {
					$original_url = '?page_id=' . $posts[0]->ID;
				} else {
					$original_url = '?post_type=' . $posts[0]->post_type . '&p=' . $posts[0]->ID;
				}
			} else {
				$post_meta = trim( strtolower( $posts[0]->meta_value ), '/' );
				if ( 'page' === $posts[0]->post_type ) {
					$get_original_url = $this->rhr_original_page_link( $posts[0]->ID );
					$original_url     = preg_replace(
						'@/+@',
						'/',
						str_replace(
							$post_meta,
							$get_original_url,
							strtolower( $request_no_slash )
						)
					);
				} else {
					$get_original_url = $this->rhr_original_post_link( $posts[0]->ID );
					$original_url     = preg_replace(
						'@/+@',
						'/',
						str_replace(
							$post_meta,
							$get_original_url,
							strtolower( $request_no_slash )
						)
					);
				}
			}
		}

		if ( null === $original_url
			|| ( null !== $original_url && ! $permalink_matched )
		) {
			// See if any terms have a matching permalink.
			$table = get_option( 'rhr_permalinks' );
			if ( $table ) {
				$rhr_term_permalink = false;
				foreach ( array_keys( $table ) as $permalink ) {
					$perm_length = strlen( $permalink );
					if ( ! $rhr_term_permalink
						&& null !== $original_url
						&& trim( $permalink, '/' ) !== $request_no_slash
					) {
						continue;
					}

					if ( substr( $request_no_slash, 0, $perm_length ) === $permalink
						|| substr( $request_no_slash . '/', 0, $perm_length ) === $permalink
					) {
						$term = $table[ $permalink ];
						$rhr_term_permalink = true;

						/*
						* Preserve this url for later if it's the same as the
						* permalink (no extra stuff).
						*/
						if ( trim( $permalink, '/' ) === $request_no_slash ) {
							$this->rhr_registered_url = $request;
						}

						$term_link    = $this->rhr_original_term_link( $term['id'] );
						$original_url = str_replace(
							trim( $permalink, '/' ),
							$term_link,
							trim( $request, '/' )
						);
					}
				}
			}
		}

		$this->rhr_rhr_parse_request_status = false;
		if ( null !== $original_url ) {
			$this->rhr_rhr_parse_request_status = true;

			$original_url = str_replace( '//', '/', $original_url );
			$pos          = strpos( $this->request_uri, '?' );
			if ( false !== $pos ) {
				$query_vars = substr( $this->request_uri, $pos + 1 );
				if ( false === strpos( $original_url, '?' ) ) {
					$original_url .= '?' . $query_vars;
				} else {
					$original_url .= '&' . $query_vars;
				}
			}

			/*
			 * Now we have the original URL, run this back through WP->rhr_parse_request,
			 * in order to parse parameters properly.
			 * We set $_SERVER variables to fool the function.
			 */
			$_SERVER['REQUEST_URI'] = '/' . ltrim( $original_url, '/' );
			$path_info              = apply_filters(
				'rhr_permalinks_path_info',
				'__false'
			);
			if ( '__false' !== $path_info ) {
				$_SERVER['PATH_INFO'] = '/' . ltrim( $original_url, '/' );
			}

			$_SERVER['QUERY_STRING'] = '';
			$pos                     = strpos( $original_url, '?' );
			if ( false !== $pos ) {
				$_SERVER['QUERY_STRING'] = substr( $original_url, $pos + 1 );
			}

			$old_values  = array();
			$query_array = array();
			if ( isset( $_SERVER['QUERY_STRING'] ) ) {
				parse_str( $_SERVER['QUERY_STRING'], $query_array );
			}

			if ( is_array( $query_array ) && count( $query_array ) > 0 ) {
				foreach ( $query_array as $key => $value ) {
					$old_values[ $key ] = '';
					if ( isset( $_REQUEST[ $key ] ) ) {
						$old_values[ $key ] = $_REQUEST[ $key ];
					}

					$_GET[ $key ]     = $value;
					$_REQUEST[ $key ] = $value;
				}
			}

			remove_filter( 'request', array( $this, 'rhr_parse_request' ) );
			global $wp;
			if ( isset( $wp->matched_rule ) ) {
				$wp->matched_rule = null;
			}
			$wp->parse_request();
			$query = $wp->query_vars;
			add_filter( 'request', array( $this, 'rhr_parse_request' ) );

			// Restore values.
			$_SERVER['REQUEST_URI']  = $this->request_uri;
			$_SERVER['QUERY_STRING'] = $this->rhr_query_string_uri;
			foreach ( $old_values as $key => $value ) {
				$_REQUEST[ $key ] = $value;
			}
		}

		return $query;
	}

	/**
	 * Filters the determined post ID and change it if we have a matching URL in CP.
	 */
	public function rhr_oembed_request( $post_id, $oembed_url ) {
		global $wpdb;

		/*
		 * First, search for a matching custom permalink, and if found
		 * generate the corresponding original URL.
		 */
		$original_url = null;
		$oembed_url   = str_replace( home_url(), '', $oembed_url );

		// Get request URI, strip parameters and /'s.
		$url     = wp_parse_url( get_bloginfo( 'url' ) );
		$url     = isset( $url['path'] ) ? $url['path'] : '';
		$request = ltrim( substr( $oembed_url, strlen( $url ) ), '/' );
		$pos     = strpos( $request, '?' );
		if ( $pos ) {
			$request = substr( $request, 0, $pos );
		}

		if ( ! $request ) {
			return $post_id;
		}

		$ignore = apply_filters( 'rhr_permalinks_request_ignore', $request );

		if ( '__true' === $ignore ) {
			return $post_id;
		}

		if ( defined( 'POLYLANG_VERSION' ) ) {
			$cp_form = new rhr_Permalinks_Form();
			$request = $cp_form->check_conflicts( $request );
		}
		$request_no_slash = preg_replace( '@/+@', '/', trim( $request, '/' ) );
		$posts            = $this->rhr_query_post( $request_no_slash );

		if ( $posts && $posts[0]->ID && $posts[0]->ID > 0 ) {
			$post_id = $posts[0]->ID;
		}

		return $post_id;
	}

	/**
	 * Action to redirect to the custom permalink.
	 */
	public function rhr_make_redirect() {
		global $wpdb;

		/*
		 * If `rhr_parse_request()` succeeded then early return to make performance
		 * better.
		 */
		if ( $this->rhr_rhr_parse_request_status ) {
			return;
		}

		if ( isset( $_SERVER['REQUEST_URI'] )
			&& $_SERVER['REQUEST_URI'] !== $this->request_uri
		) {
			$this->request_uri = $_SERVER['REQUEST_URI'];
		}

		$rhr_permalink   = '';
		$original_permalink = '';

		// Get request URI, strip parameters.
		$url     = wp_parse_url( get_bloginfo( 'url' ) );
		$url     = isset( $url['path'] ) ? $url['path'] : '';
		$request = ltrim( substr( $this->request_uri, strlen( $url ) ), '/' );
		$pos     = strpos( $request, '?' );
		if ( $pos ) {
			$request = substr( $request, 0, $pos );
		}

		/*
		 * Disable redirects to be processed if filter returns `true`.
		 */
		$avoid_redirect = apply_filters(
			'rhr_permalinks_avoid_redirect',
			$request
		);

		if ( is_bool( $avoid_redirect ) && $avoid_redirect ) {
			return;
		}

		if ( defined( 'POLYLANG_VERSION' ) ) {
			$cp_form = new rhr_Permalinks_Form();
			$request = $cp_form->check_conflicts( $request );
		}
		$request_no_slash = preg_replace( '@/+@', '/', trim( $request, '/' ) );
		$posts            = $this->rhr_query_post( $request_no_slash );

		if ( ! isset( $posts[0]->ID ) || ! isset( $posts[0]->meta_value )
			|| empty( $posts[0]->meta_value )
		) {
			global $wp_query;

			/*
			 * If the post/tag/category we're on has a custom permalink, get it
			 * and check against the request.
			 */
			if ( ( is_single() || is_page() ) && ! empty( $wp_query->post ) ) {
				$post             = $wp_query->post;
				$rhr_permalink = get_post_meta(
					$post->ID,
					'rhr_permalink',
					true
				);
				if ( 'page' === $post->post_type ) {
					$original_permalink = $this->rhr_original_page_link( $post->ID );
				} else {
					$original_permalink = $this->rhr_original_post_link( $post->ID );
				}
			} elseif ( is_tag() || is_category() ) {
				$the_term           = $wp_query->get_queried_object();
				$rhr_permalink   = $this->rhr_term_permalink( $the_term->term_id );
				$original_permalink = $this->rhr_original_term_link( $the_term->term_id );
			}
		} else {
			$rhr_permalink = $posts[0]->meta_value;
			if ( 'page' === $posts[0]->post_type ) {
				$original_permalink = $this->rhr_original_page_link( $posts[0]->ID );
			} else {
				$original_permalink = $this->rhr_original_post_link( $posts[0]->ID );
			}
		}

		$custom_length = strlen( $rhr_permalink );
		if ( $rhr_permalink
			&& (
				substr( $request, 0, $custom_length ) !== $rhr_permalink
				|| $request === $rhr_permalink . '/'
			)
		) {
			// Request doesn't match permalink - redirect.
			$url             = $rhr_permalink;
			$original_length = strlen( $original_permalink );

			if ( substr( $request, 0, $original_length ) === $original_permalink
				&& trim( $request, '/' ) !== trim( $original_permalink, '/' )
			) {
				// This is the original link; we can use this url to derive the new one.
				$url = preg_replace(
					'@//*@',
					'/',
					str_replace(
						trim( $original_permalink, '/' ),
						trim( $rhr_permalink, '/' ),
						$request
					)
				);
				$url = preg_replace( '@([^?]*)&@', '\1?', $url );
			}

			// Append any query compenent.
			$url .= strstr( $this->request_uri, '?' );

			wp_safe_redirect( home_url() . '/' . $url, 301 );
			exit( 0 );
		}
	}

	/**
	 * Filter to replace the post permalink with the custom one.
	 */
	public function rhr_custom_post_link( $permalink, $post ) {
		$rhr_permalink = get_post_meta( $post->ID, 'rhr_permalink', true );
		if ( $rhr_permalink ) {
			$post_type = 'post';
			if ( isset( $post->post_type ) ) {
				$post_type = $post->post_type;
			}

			$language_code = apply_filters(
				'wpml_element_language_code',
				null,
				array(
					'element_id'   => $post->ID,
					'element_type' => $post_type,
				)
			);

			$permalink = $this->rhr_wpml_permalink_filter(
				$rhr_permalink,
				$language_code
			);
		} else {
			if ( class_exists( 'SitePress' ) ) {
				$wpml_lang_format = apply_filters(
					'wpml_setting',
					0,
					'language_negotiation_type'
				);

				if ( 1 === intval( $wpml_lang_format ) ) {
					$get_original_url = $this->rhr_original_post_link( $post->ID );
					$permalink        = $this->rhr_remove_double_slash( $permalink );
					if ( strlen( $get_original_url ) === strlen( $permalink ) ) {
						$permalink = $get_original_url;
					}
				}
			}
		}

		$permalink = $this->rhr_remove_double_slash( $permalink );

		return $permalink;
	}

	/**
	 * Filter to replace the page permalink with the custom one.
	 */
	public function rhr_custom_page_link( $permalink, $page ) {
		$rhr_permalink = get_post_meta( $page, 'rhr_permalink', true );
		if ( $rhr_permalink ) {
			$language_code = apply_filters(
				'wpml_element_language_code',
				null,
				array(
					'element_id'   => $page,
					'element_type' => 'page',
				)
			);

			$permalink = $this->rhr_wpml_permalink_filter(
				$rhr_permalink,
				$language_code
			);
		} else {
			if ( class_exists( 'SitePress' ) ) {
				$wpml_lang_format = apply_filters(
					'wpml_setting',
					0,
					'language_negotiation_type'
				);

				if ( 1 === intval( $wpml_lang_format ) ) {
					$get_original_url = $this->rhr_original_page_link( $page );
					$permalink        = $this->rhr_remove_double_slash( $permalink );
					if ( strlen( $get_original_url ) === strlen( $permalink ) ) {
						$permalink = $get_original_url;
					}
				}
			}
		}

		$permalink = $this->rhr_remove_double_slash( $permalink );

		return $permalink;
	}

	/**
	 * Filter to replace the term permalink with the custom one.
	 */
	public function rhr_custom_term_link( $permalink, $term ) {
		if ( isset( $term ) ) {
			if ( isset( $term->term_id ) ) {
				$rhr_permalink = $this->rhr_term_permalink( $term->term_id );
			}

			if ( $rhr_permalink ) {
				$language_code = null;
				if ( isset( $term->term_taxonomy_id ) ) {
					$term_type = 'category';
					if ( isset( $term->taxonomy ) ) {
						$term_type = $term->taxonomy;
					}

					$language_code = apply_filters(
						'wpml_element_language_code',
						null,
						array(
							'element_id'   => $term->term_taxonomy_id,
							'element_type' => $term_type,
						)
					);
				}

				$permalink = $this->rhr_wpml_permalink_filter(
					$rhr_permalink,
					$language_code
				);
			} elseif ( isset( $term->term_id ) ) {
				if ( class_exists( 'SitePress' ) ) {
					$wpml_lang_format = apply_filters(
						'wpml_setting',
						0,
						'language_negotiation_type'
					);

					if ( 1 === intval( $wpml_lang_format ) ) {
						$get_original_url = $this->rhr_original_term_link(
							$term->term_id
						);
						$permalink        = $this->rhr_remove_double_slash( $permalink );
						if ( strlen( $get_original_url ) === strlen( $permalink ) ) {
							$permalink = $get_original_url;
						}
					}
				}
			}
		}

		$permalink = $this->rhr_remove_double_slash( $permalink );

		return $permalink;
	}

	/**
	 * Remove the post_link and user_trailingslashit filter to get the original
	 * permalink of the default and custom post type and apply right after that.
	 */
	public function rhr_original_post_link( $post_id ) {
		remove_filter( 'post_link', array( $this, 'rhr_custom_post_link' ) );
		remove_filter( 'post_type_link', array( $this, 'rhr_custom_post_link' ) );

		$post_file_path = ABSPATH . '/wp-admin/includes/post.php';
		include_once $post_file_path;

		list( $permalink, $post_name ) = get_sample_permalink( $post_id );
		$permalink                     = str_replace(
			array( '%pagename%', '%postname%' ),
			$post_name,
			$permalink
		);
		$permalink                     = ltrim( str_replace( home_url(), '', $permalink ), '/' );

		add_filter( 'post_link', array( $this, 'rhr_custom_post_link' ), 10, 3 );
		add_filter( 'post_type_link', array( $this, 'rhr_custom_post_link' ), 10, 2 );

		return $permalink;
	}

	/**
	 * Remove the page_link and user_trailingslashit filter to get the original
	 * permalink of the page and apply right after that.
	 */
	public function rhr_original_page_link( $post_id ) {
		remove_filter( 'page_link', array( $this, 'rhr_custom_page_link' ) );
		remove_filter(
			'user_trailingslashit',
			array( $this, 'rhr_custom_trailingslash' )
		);

		$post_file_path = ABSPATH . '/wp-admin/includes/post.php';
		include_once $post_file_path;

		list( $permalink, $post_name ) = get_sample_permalink( $post_id );
		$permalink                     = str_replace(
			array( '%pagename%', '%postname%' ),
			$post_name,
			$permalink
		);
		$permalink                     = ltrim( str_replace( home_url(), '', $permalink ), '/' );

		add_filter( 'user_trailingslashit', array( $this, 'rhr_custom_trailingslash' ) );
		add_filter( 'page_link', array( $this, 'rhr_custom_page_link' ), 10, 2 );

		return $permalink;
	}

	/**
	 * Remove the term_link and user_trailingslashit filter to get the original
	 * permalink of the Term and apply right after that.
	 */
	public function rhr_original_term_link( $term_id ) {
		remove_filter( 'term_link', array( $this, 'rhr_custom_term_link' ) );
		remove_filter(
			'user_trailingslashit',
			array( $this, 'rhr_custom_trailingslash' )
		);

		$term      = get_term( $term_id );
		$term_link = get_term_link( $term );

		add_filter( 'user_trailingslashit', array( $this, 'rhr_custom_trailingslash' ) );
		add_filter( 'term_link', array( $this, 'rhr_custom_term_link' ), 10, 2 );

		if ( is_wp_error( $term_link ) ) {
			return '';
		}

		$original_permalink = ltrim( str_replace( home_url(), '', $term_link ), '/' );

		return $original_permalink;
	}

	/**
	 * Filter to handle trailing slashes correctly.
	 */
	public function rhr_custom_trailingslash( $url_string ) {
		remove_filter(
			'user_trailingslashit',
			array( $this, 'rhr_custom_trailingslash' )
		);

		$trailingslash_string = $url_string;
		$url                  = wp_parse_url( get_bloginfo( 'url' ) );

		if ( isset( $url['path'] ) ) {
			$request = substr( $url_string, strlen( $url['path'] ) );
		} else {
			$request = $url_string;
		}

		$request = ltrim( $request, '/' );

		add_filter( 'user_trailingslashit', array( $this, 'rhr_custom_trailingslash' ) );

		if ( trim( $request ) ) {
			if ( trim( $this->rhr_registered_url, '/' ) === trim( $request, '/' ) ) {
				if ( '/' === $url_string[0] ) {
					$trailingslash_string = '/';
				} else {
					$trailingslash_string = '';
				}

				if ( isset( $url['path'] ) ) {
					$trailingslash_string .= trailingslashit( $url['path'] );
				}

				$trailingslash_string .= $this->rhr_registered_url;
			}
		}

		return $trailingslash_string;
	}

	/**
	 * Get permalink for term.
	 */
	public function rhr_term_permalink( $term_id ) {
		$table = get_option( 'rhr_permalinks' );
		if ( $table ) {
			foreach ( $table as $link => $info ) {
				if ( $info['id'] === $term_id ) {
					return $link;
				}
			}
		}

		return false;
	}

	/**
	 * Fix double slash issue with canonical of Yoast SEO specially with WPML.
	 */
	public function rhr_fix_canonical_double_slash( $canonical ) {
		$canonical = $this->rhr_remove_double_slash( $canonical );

		return $canonical;
	}
}
new rhr_permalink_frontend();
