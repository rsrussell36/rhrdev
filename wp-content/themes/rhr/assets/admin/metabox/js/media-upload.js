; (function ($, window, document, undefined) {
    ...
    $.rhrMediaInsert = function (el, options) {
        this.el = el;
        this.$el = $(el);
        this.selectedItems = '';
        this.selectedUrl = [];
    ...
    $.rhrMediaInsert.prototype = {
        ...
        insertSavedItems: function () {
            var self = this;
            var con = self.$el.parent('.rhr-container');
            var selectedImage = con.find('.rhr-saved-val').val();
            if (selectedImage != '') {

                var savedItems = $.parseJSON(con.find('.rhr-saved-val').val())
            }
            ...
            if (savedItems.length > 0) {
                if (self.options.fileType == "image") {
                    ...
                    for (var i in savedItems) {
                        ...
                        //self.selectedUrl
                        itemsCon.append('<div class="selected-image" id="img-' + i + '"><img src="' + savedItems[i]['thumb'] + '" class="rhr_images" data-full="' + savedItems[i]['full'] + '" data-itemid="' + savedItems[i]['itemId'] + '" data-id="' + [i] + '" alt=""><a href="#" class="remove">Remove</a></div>');
                    }
        ...
        openMedia: function () {
            var rhr_file_frame, attachment, self = this, container = this.$el.prev('.selected-con');
            ...
            this.$el.on('click', function (e) {
                ...
                rhr_file_frame = wp.media.frames.rhr_file_frame = wp.media({
                    title: uploaderTitle,
                    button: {
                ...
                rhr_file_frame.on('open', function () {
                    var selection = rhr_file_frame.state().get('selection');
                ...
                rhr_file_frame.on('select', function () {
                    ...
                    if (!self.options.multiSelect) {
                        attachment = rhr_file_frame.state().get('selection').first().toJSON();
                        self.selectedUrl = [];
        ...
        buildImageFrag: function (attachment, btn) {
            ...
            img = $('<img>', { src: imgUrl, class: 'rhr_images', 'data-full': fullSize, 'data-id': this.i, 'data-itemid': attachment.id });
        ...
        bindEvents: function () {
            ...
            $('.remove').on('click', function (e) {
                ...
                if (self.options.fileType == "image") {
                    ...
                    id = selImage.find('.rhr_images').data(id);
            ...
            this.$el.prev('.selected-con').sortable({
                ...
                update: function () {
                    ...
                    $.each(newOrder, function (i, item) {
                        var id = $('#' + item).find('.rhr_images').data('id');
                        if (oldOrder[id]) {
    ...
    $.fn.rhrMediaInsert = function (options) {

        return this.each(function () {
            // prevent multiple instantiation
            if (!$(this).data('rhrmediainsert'))
                $(this).data('rhrmediainsert', new $.rhrMediaInsert(this, options));
        });
    };
